    ...
    <div className="w-full h-full flex flex-col items-center justify-center">
      <div className="relative flex items-center justify-center w-full h-full" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block' }}>
          {/* Whiskers */}
          <line x1={scale(min)} y1={h/2} x2={scale(q1)} y2={h/2} stroke="#3B82F6" strokeWidth={2} />
          <line x1={scale(q3)} y1={h/2} x2={scale(max)} y2={h/2} stroke="#3B82F6" strokeWidth={2} />
          {/* Whisker caps */}
          <line x1={scale(min)} y1={h/2-12} x2={scale(min)} y2={h/2+12} stroke="#3B82F6" strokeWidth={2} />
          <line x1={scale(max)} y1={h/2-12} x2={scale(max)} y2={h/2+12} stroke="#3B82F6" strokeWidth={2} />
          {/* Box */}
          <rect x={scale(q1)} y={h/2-18} width={scale(q3)-scale(q1)} height={36} rx={8} fill="#475569" opacity={0.95} />
          {/* Median */}
          <rect x={scale(median)-2} y={h/2-18} width={4} height={36} rx={2} fill="#3B82F6" opacity={0.95} />
          {/* Outliers */}
          {outliers.map((o, i) => (
            <circle key={i} cx={scale(o)} cy={h/2} r={6} fill="#F59E42" />
          ))}
        </svg>
        {isHovered && (
