...
export const Dashboard = () => {
  ...
  { name: "May", value: 7 }
        ...
        <Dialog open={!!expandedCard} onOpenChange={() => setExpandedCard(null)}>
          <DialogContent className="max-w-4xl w-full h-[70vh] flex">
            <div className="w-1/2 flex items-center justify-center p-6 border-r border-border">
              {expandedCard && (
                <DashboardLineChart data={editData} cardTitle={editableCards.find(c => c.key === expandedCard)?.cardTitle} />
              )}
            </div>
            <div className="w-1/2 p-6 overflow-auto">
              <h2 className="text-lg font-bold mb-4">Edit Trend Data</h2>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {editData && editData.map((row: any, idx: number) => (
